...
class WebDownloaderGUI:
    ...
    def __init__(self, root):
        ...
        self.theme = None
        try:
            from ui_modern_theme import ModernTheme
            ModernTheme.apply(self.root)
            self.theme = ModernTheme
            self.root.configure(bg=ModernTheme.COLORS['bg_dark'])
    ...
    def setup_styles(self):
        ...
        if self.theme:
            ...
            style.configure("Success.TButton",
                           font=("Segoe UI", 10, "bold"),
                           foreground=ModernTheme.COLORS['bg_dark'],
                           background=ModernTheme.COLORS['success'])
            style.configure("Frame.TFrame", background=ModernTheme.COLORS['bg_dark'])
            ...
            style.configure("Log.TFrame",
                           background=ModernTheme.COLORS['bg_medium'])
    ...
    def create_header(self, parent):
        ...
        title_label = ttk.Label(left_section, 
                               text="🌐 网页资源离线下载器",
                               font=("Segoe UI", 16, "bold"),
                               foreground=self.theme.COLORS['text_primary'])
        title_label.pack(side="left")
        
        version_label = ttk.Label(left_section,
                                 text="  v2.0 Pro",
                                 font=("Segoe UI", 10),
        ...
        manual_btn = ttk.Button(right_section, text="📖 用户手册", command=self.show_user_manual, width=12)
        manual_btn.pack(side="right", padx=(5, 0))
        
        about_btn = ttk.Button(right_section, text="ℹ️ 关于", command=self.show_about, width=8)
    ...
    def create_right_column_content(self, parent):
        ...
        if self.theme:
            log_bg = self.theme.COLORS['bg_medium']
            log_fg = self.theme.COLORS['text_primary']
            log_insert = self.theme.COLORS['accent']
            log_select = self.theme.COLORS['accent']
            success_color = self.theme.COLORS['success']
            error_color = self.theme.COLORS['error']
            warning_color = self.theme.COLORS['warning']
            info_color = self.theme.COLORS['accent']
        else:
            log_bg = "#f8f9fa"
            log_fg = "#2c3e50"
            log_insert = "#3498db"
        ...
        self.log_area.tag_config("success", foreground=success_color)
        self.log_area.tag_config("error", foreground=error_color)
        self.log_area.tag_config("warning", foreground=warning_color)
        self.log_area.tag_config("info", foreground=info_color)
...
请通过正规渠道购买，支持正版！"""
    ...
    def create_status_bar(self, parent):
        status_frame = ttk.Frame(parent, relief="flat")
        ...
        status_label = ttk.Label(status_frame, 
                                ...
                                font=("Segoe UI", 9),
                                style="Status.TLabel")
        status_label.pack(side="left", fill="x", expand=True, pady=2)
        ...
        if self.trial_remaining > 0 and self.trial_color:
            trial_label = tk.Label(status_frame,
                                  ...
                                  fg=self.trial_color,
                                  padx=10)
            trial_label.pack(side="right", pady=2)
